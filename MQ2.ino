...
{ 
  ...
  gw.sendSketchInfo("MQ-2 Sensor", "1.0");

  // Register all sensors to gateway (they will be created as child devices)
  gw.sendSensorPresentation(CHILD_ID_MQ, 22);  

  Ro = MQCalibration(MQ2_SENSOR_ANALOG_PIN);         //Calibrating the sensor. Please make sure the sensor is in clean air 
...
{     
  ...
  if (valMQ != lastMQ) {
      gw.sendVariable(CHILD_ID_MQ, V_VAR1, (int)ceil(valMQ));
      lastMQ = ceil(valMQ);
  }
